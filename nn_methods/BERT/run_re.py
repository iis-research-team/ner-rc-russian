...
from utils_re import convert_examples_to_features, get_labels, read_examples_from_file, ADDITIONAL_SPECIAL_TOKENS
from bert_for_re import BertRE, BertREConfig
...
def evaluate(args, model, tokenizer, labels, mode, prefix=""):
    ...
    preds_list = [label_map[p] for p in preds]
...
def load_and_cache_examples(args, tokenizer, mode):
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_features(examples, args.max_seq_length, tokenizer, get_labels(args.labels))
        if args.local_rank in [-1, 0]:
...
def main():
    ...
    # Other parameters
    parser.add_argument("--dropout_rate", type=int, default=0.1, help="Dropout for model")
    parser.add_argument(
        "--labels",
        default="",
        type=str,
    ...
    config = BertREConfig.from_pretrained(
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, _ = evaluate(args, model, tokenizer, labels, mode="dev", prefix=global_step)
            if global_step:
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        model = BertRE.from_pretrained(args.output_dir)
            ...
            with open(os.path.join(args.data_dir, "test.txt"), "r") as f:
                ...
                for line in f:
                    if line.startswith("-DOCSTART-") or line == "" or line == "\n":
                        ...
                        output_line = line + " " + predictions[example_id] + "\n"
